    ...
    <div id="ecrit-page" className="h-full overflow-y-auto py-6 px-2 md:px-5 md:py-8">
      <div className="mx-auto flex w-full max-w-full flex-col gap-6">
        <form id="ecrit-form" onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {/* ── Étape 2 : Observations et contenu ── */}
          <StepCard
            step="2"
            title="Observations et contenu"
            subtitle="Décrivez librement les faits, vos observations et analyses — l'IA structure l'écrit éducatif"
            accentColor={ACCENT}
            headerAction={
          ...
          >
            <TranscriptionCard
              ...
              placeholder="Décrivez les faits observés, vos analyses, les éléments de contexte, les progrès réalisés, les difficultés rencontrées, les pistes d'action ou préconisations… L'IA se charge du reste."
              rows={8}
              disabled={loading}
            />
          ...
          <div id="form-actions" className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Button type="submit" color="rose" size="lg" disabled={loading || !observations.trim()} className="flex-1">
                {loading ? "Génération en cours…" : "Générer l'écrit éducatif"}
              </Button>
              <Button color="rose" size="lg" icon={FilePlus} onClick={handleReset} className="flex-1">
                Nouvel écrit
              </Button>
            </div>
...
export default EcritEducatifPage;
