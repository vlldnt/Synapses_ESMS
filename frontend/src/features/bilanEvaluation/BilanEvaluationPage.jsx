...
function BilanEvaluationPage() {
  ...
  const handleReset = () => {
    if (!window.confirm("Commencer un nouveau bilan ? Le brouillon sera effacé.")) return;
  ...
  return (
    <div id="bilan-page" className="h-full overflow-y-auto py-6 px-2 md:px-5 md:py-8">
      <div className="mx-auto flex w-full max-w-full flex-col gap-6">
        <form id="bilan-form" onSubmit={handleSubmit} className="flex flex-col gap-6">

          {/* ── Étape 1 : Contexte ── */}
          <StepCard step="1" title="Contexte" subtitle="Bilan d'évaluation" accentColor={ACCENT}>
            {/* Desktop */}
          ...
          {/* ── Étape 2 : Éléments d'évaluation ── */}
          <StepCard
            step="2"
            title="Éléments d'évaluation"
            subtitle="Décrivez les observations, les évolutions et les axes à évaluer — l'IA structure le bilan"
            accentColor={ACCENT}
            headerAction={
          ...
          <div id="form-actions" className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Button type="submit" color="violet" size="lg" disabled={loading || !observations.trim()} className="flex-1">
                {loading ? "Génération en cours…" : "Générer le bilan"}
              </Button>
              <Button color="violet" size="lg" icon={FilePlus} onClick={handleReset} className="flex-1">
                Nouveau bilan
              </Button>
            </div>
        ...
        {result && (
          <GeneratedResult
            id="bilan-result"
            title="Bilan d'évaluation généré"
            result={result}
            validated={validated}
...
export default BilanEvaluationPage;
