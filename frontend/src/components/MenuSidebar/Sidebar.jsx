...
function Sidebar() {
  return (
    <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 bg-(--bg-primary) shadow-sm flex-col z-50 border-r border-(--border)">
      {/* Logo + Titre */}
      <Link className="flex flex-col justify-center items-start gap-3 px-5 py-6" to="/">
      ...
      <nav className="flex-1 py-4 mt-20">
        <p className="flex text-(--text-muted) mb-2 px-3 mt-2">Module IA</p>
        <ul className="flex flex-col gap-1 list-none mb-10">
          ...
          />
          <NavItem titre="Compte rendu" link="/compte-rendu" icon={ClipboardPen} />
          <NavItem
        ...
        <ul className="flex flex-col gap-1 list-none mb-4">
          <NavItem titre="Historique" link="/historique" icon={LayoutDashboard} />
        </ul>
      </nav>
