...
router.post("/organization-requests", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Un champ dépasse la longueur maximale autorisée." });
  }
...
router.get("/organization-requests/info/:token", async (req, res) => {
  try {
    ...
    const request = requests.find(
      (r) => r.verification_token === req.params.token,
    );

    if (!request)
      return res.status(404).json({ error: "Lien invalide ou déjà utilisé." });
    if (request.status !== "pending_verification")
      return res.status(409).json({ error: "Ce compte a déjà été créé." });
    if (new Date(request.verification_expiry) < new Date())
...
router.get("/user-requests/info/:token", async (req, res) => {
  try {
    ...
    const inv = invitations.find(
      (i) => i.verification_token === req.params.token,
    );
    if (!inv)
      return res.status(404).json({ error: "Lien invalide ou déjà utilisé." });
    if (inv.status !== "pending")
      return res.status(409).json({ error: "Ce compte a déjà été créé." });
    if (new Date(inv.verification_expiry) < new Date())
      return res.status(410).json({ error: "Ce lien a expiré." });
...
router.post("/organization-requests/:id/approve", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ organization: newOrg, user: safeUser, tempPassword });
  } catch (err) {
